...
#
# ということは、確かめるべきは「systemd が終了コード4 を成功として扱うか」
# である。fsck を偽物に差し替えて 4 を返させる方式は採らない。対策が
# 効いていなかった場合に、抜け出せない再起動の繰り返しになるため (段階1 の
# 説明を参照)。代わりに、同じ SuccessExitStatus を持つ試験ユニットを
# 終了コード4 で終わらせ、持たない対照と比べる。
#
