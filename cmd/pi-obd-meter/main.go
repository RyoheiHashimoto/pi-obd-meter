...
// なり、常に false を返すようになった。実際には疎通しており、判定だけが
// 壊れていた。
//
// 影響は initializeFromGAS の初回送信がスキップされることに限られる。
// 5分周期の送信 (maintTicker → sendMaintenanceStatusAsync) はこの判定を
// 経由しないため、初回がスキップされても後から送信される。
//
