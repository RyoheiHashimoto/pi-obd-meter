...
- 右パネル: バキューム計 (-1.0〜0 Bar、MAP kPa を変換) + 4行インジケーター (ECO/TEMP/TRIP/OIL)
- VACUUM ラベルが負圧に応じて暗→明→赤+グロー
...
│       ├── js/
│       │   ├── main.js       # エントリポイント + WebSocket/HTTPポーリング
│       │   ├── gauge.js      # 速度ゲージ(針+アーク) + RPMアーク + スロットルアーク + ギア/レンジ
...
│   ├── capture-screenshots.sh # メーター画面の定期スクリーンショット
