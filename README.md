...
│   └── appsscript.json    # GAS マニフェスト（タイムゾーン・ランタイム等）
├── configs/
│   ├── config.json
...
`gas/` 配下のファイルを main に push すると GitHub Actions が自動で:
1. ESLint で構文チェック（エラーがあればデプロイ中止）
2. `clasp push` で HEAD デプロイメントに反映
3. `clasp deploy` で本番 Web アプリ URL に反映

```bash
make deploy-gas      # ローカルから手動で GAS に push（CI を待たずに即反映したい場合）
```

初回セットアップ時のみ `clasp login` による Google OAuth 認証が必要。
