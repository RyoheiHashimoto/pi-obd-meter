...
車種によって搭載センサーが異なるため、2つの計算方式を用意している。起動時に PID 0x00 でサポートPIDを自動スキャンし、最適な方式を自動選択する（設定不要）。

| 方式 | 必要センサー | 精度 | 対象車種 |
|------|------------|------|---------|
| **MAF方式** | MAFセンサー (PID 0x10) | ★★★ | エアフロセンサー搭載車（多くの国産車・輸入車） |
| **MAP方式** | MAPセンサー (PID 0x0B) + 吸気温度 (PID 0x0F) | ★★☆ | エアフロ非搭載車（一部の小排気量車・軽自動車等） |
