...
- **develop**: 日常開発ブランチ。`make deploy` でPiに直接転送。CI（lint + test）実行
- **main**: リリースブランチ。タグ push でGitHub Releaseを作成、Pi自動更新
- **GASデプロイ**: main push時のみ（`gas/` 変更時）

### ワークフロー
1. develop で開発・テスト・`make deploy`
