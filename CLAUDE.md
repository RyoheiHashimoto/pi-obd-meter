...
- **develop**: 日常開発ブランチ。push → CI + dev ビルド自動デプロイ
- **main**: リリースブランチ。タグ push でGitHub Releaseを作成
- **GASデプロイ**: develop/main push時（`gas/` 変更時）
- **Pi 自動更新**: 2分間隔で GitHub をポーリング、新ビルド検出時に自動更新

### ワークフロー
1. develop で開発・`make deploy`（即時）または push（2分以内に自動デプロイ）
2. リリース時: `make release` (develop→main自動マージ + タグ push)
3. GitHub Actions が ARM64 バイナリをビルド・リリース
4. Pi が2分以内に検出して自動更新

### Pi 自動更新の仕組み
- `auto-update.timer` (systemd) が2分間隔で `auto-update.sh` を実行
...
# リリース（develop→main マージ + タグ push → GitHub Actions → Pi 自動更新）
make release         # パッチ自動インクリメント (v0.3.1 → v0.3.2)
make release V=v1.0.0  # バージョン明示指定

# GAS デプロイ (clasp — main push時にCI/CDで自動実行)
